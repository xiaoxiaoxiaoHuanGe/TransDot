...
- 使用二维码或 6 位备用码配对浏览器。
- Android 与最新版 Chrome/Edge 位于同一局域网时，可使用“局域网快传”让文件直接在两台设备之间传输。
...
局域网快传是独立于云端时间线的直接传输模式。Android 和电脑浏览器仍需同时连接同一台 TransDot 服务器，服务器只通过现有 HTTPS/WSS `/ws` 协调连接；文件名、MIME、大小、哈希和文件内容只在 WebRTC DataChannel 中传递，不写入服务器消息、数据库或文件卷。

使用步骤：

1. 确保 Android 和电脑连接同一个非隔离局域网，并已正常登录同一个 TransDot 实例。
2. 电脑使用最新版 Chrome 或 Edge 打开 Web 页面，点击“局域网快传”。
3. 首次使用时在 Web 端点击“选择接收文件夹”并授权；浏览器会保存目录句柄，权限被撤销后需重新授权。
4. Android 时间线顶栏点击“快传”。若尚未设置默认保存目录，先点击“选择接收文件夹”；之后手机会自动接收。
5. 任意一端可多选文件，队列会逐个传输。传输期间 Android 显示前台通知，可从页面或通知取消。

限制与行为：

- 每批最多 20 个文件，单文件最大 2 GiB，使用 64 KiB 分块并在完成前校验 SHA-256。
- 不支持断点续传；取消、断线、校验失败会删除接收端未完成文件。
- 8 秒内无法建立局域网直连就明确失败，绝不会自动改走云端上传。修复网络后可重新连接和重试。
- 只使用 Host ICE，不配置 STUN/TURN，不增加服务器端口。路由器 AP/客户端隔离、访客 Wi-Fi、VPN 或设备防火墙可能阻止直连。
- Web 接收目录能力要求最新版 Chrome/Edge 和安全上下文；公网部署继续使用 HTTPS。

普通云端时间线仍按原来的容量和保留策略工作，局域网快传不受 `MAX_FILE_BYTES` 或文件池容量限制。普通代码升级不需要 Reset，也不会要求重新扫码初始化。
